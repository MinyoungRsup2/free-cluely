...
export class RealtimeScreenHelper {
  ...
  public async analyzeCurrentScreenForActions(): Promise<DetectedAction[]> {
    ...
    try {
      ...
      const result = await (llmHelper as any).model.generateContent([prompt, imagePart])
  ...
  private async getNameFromScreen(): Promise<string> {
    try {
      const imageDataUrl = await this.captureCurrentScreen()
  ...
  private extractPatientId(detectedInfo: string): string | null {
    // Extract patient ID from format: "Name: Minyoungo, ID: 27103"
    const idMatch = detectedInfo.match(/ID:\s*(\d+)/i)
    return idMatch ? idMatch[1] : null
  }

  private async findConsumerByEhrPatientId(ehrPatientId: string): Promise<any> {
    try {
  ...
  private async navigateToCompanionUrl(consumerId: string): Promise<void> {
    try {
